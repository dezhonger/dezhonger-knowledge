import { labelOf, partOfSpeechLabel } from './model.mjs'
export function practiceData(model) {
  const words = model.vocabulary.map(word => ({
    ...
    meanings: word.senses.map(sense => ({ pos: partOfSpeechLabel(model.taxonomy.partsOfSpeech, sense.posId), posTitle: labelOf(model.taxonomy.partsOfSpeech, sense.posId), zh: sense.zh })),
  }))
