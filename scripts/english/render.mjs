...
${path !== '/' ? `<nav class="desktop-nav" aria-label="主导航">${nav.map(([href, label]) => `<a href="${href}"${path === href || (href !== '/' && path.startsWith(href + '/')) ? ' aria-current="page"' : ''}>${label}</a>`).join('')}</nav>` : ''}
<div class="header-actions"><a class="header-search" href="/search" aria-label="搜索"${section === 'search' ? ' aria-current="page"' : ''}>${icon('search', 18)}<span>搜索</span></a>${path !== '/' ? `<a class="header-practice" href="/vocabulary/practice">随机背词 ${icon('arrow', 15)}</a><button class="menu-toggle" aria-label="展开导航" aria-expanded="false" aria-controls="mobile-nav" hidden>${icon('menu')}</button>` : ''}</div>
</div>${path !== '/' ? `<nav id="mobile-nav" class="mobile-nav" aria-label="手机导航" hidden>${nav.map(([href, label]) => link(href, label)).join('')}${link('/vocabulary/practice', '随机背词')}</nav><noscript><nav class="mobile-nav" aria-label="手机导航">${nav.map(([href, label]) => link(href, label)).join('')}${link('/vocabulary/practice', '随机背词')}</nav></noscript>` : ''}</header>
<main id="main" class="page${path === '/' ? ' page--home' : ''}" tabindex="-1">${crumbs.length ? breadcrumb(crumbs) : ''}${body}</main>
<footer class="site-footer"><div><a class="footer-brand" href="/">English Learning<span>学一点，用一点。</span></a><p>Learn at your own pace.</p>${path !== '/' ? `<nav aria-label="页脚导航"><a href="/vocabulary/practice">随机背词</a><a href="/sources">词表来源</a></nav>` : ''}</div></footer>
...
export function moduleCards(headingLevel = 3) {
  return `<div class="module-grid">${SECTIONS.map(s => `<a class="module-card module-card--${s.id}" href="${s.path}"><div class="module-copy"><p class="module-en">${s.en}</p><h${headingLevel}>${s.label}</h${headingLevel}><p>${s.description}</p><span class="module-cta">开始学习 ${icon('arrow', 17)}</span></div>${illustration(s.id)}</a>`).join('')}</div>`
}
...
export function home() {
  return `<header class="home-hero"><h1>英语学习</h1><p class="hero-copy">选择一个分类，开始学习。</p></header><section class="home-section home-categories" aria-label="学习分类">${moduleCards(2)}</section>`
}
