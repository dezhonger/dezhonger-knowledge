...
if (built) {
  for (const prefix of ['', 'zh/']) {
    ...
    assert.equal($('#rosecode-pagination .page-numbers button[aria-current="page"]').text(), '1')
    assert.equal($('#rosecode-pagination .page-numbers button').last().text(), '57')
    assert.equal($('#rosecode-pagination .page-actions--start button[disabled]').length, 2)
    assert.equal($('#rosecode-pagination .page-actions--end button[disabled]').length, 0)
    assert.equal($('#rosecode-pagination-jump').attr('value'), '1')
    assert.ok($('#rosecode-pagination-range').text().includes('57'))
