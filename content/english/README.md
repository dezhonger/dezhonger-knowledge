...
- `vocabulary/*.jsonl`：一行一个完整 JSON 词条，公开词表按字母分片；`food.jsonl` 和 `nature.jsonl` 分别收录水果蔬菜、动物植物主题新增词。
...
总共 12,644 个去重词条。所有指定源词表均完整纳入。“完整”指覆盖这些固定版本的公开词表，不表示所有地区、教材或考试存在一个官方唯一词汇大纲。
...
英式音标覆盖 11,233 个词，美式覆盖 11,602 个词，两者都有的为 10,845 个。剩余缺项保留清单，不伪造读音。
