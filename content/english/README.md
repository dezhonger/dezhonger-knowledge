...
- `vocabulary/*.jsonl`：按字母分片，一行一个完整 JSON 词条；大规模导入后只有 26 个分片。
...
编辑既有导入词时，找到对应 JSONL 的单行。若需长期精编，把这一行移到独立 `<id>.json`，从原分片删除该行，再格式化编辑。导入脚本会保留独立 JSON 的释义和例句，并补充公开词表归属。
...
总共 11,496 个去重词条。所有指定源词表均完整纳入。“完整”指覆盖这些固定版本的公开词表，不表示所有地区、教材或考试存在一个官方唯一词汇大纲。
...
英式音标覆盖 10,601 个词，美式覆盖 11,096 个词，两者都有的为 10,366 个。剩余缺项保留清单，不伪造读音。
