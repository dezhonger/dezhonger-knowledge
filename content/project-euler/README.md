...
题库按同步时的快照发布，目前不自动定时同步。官网在采集后发布的新题，需要重新同步、补齐中文翻译并发布。即使本地构建已通过，发布前仍须运行下方的 `--latest` 检查，避免制作译文期间有新题发布而遗漏。
...
# 发布前必做：同时联网核对官网最新题号与近期标题
npm run validate:project-euler -- --built --latest
```

`--latest` 只读官网最新目录，不改写本地内容。若出现新题、近期标题变化或官网无法读取，检查失败，应完成同步与翻译后重新构建、校验，再发布。
