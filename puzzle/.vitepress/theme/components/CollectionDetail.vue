...
const { page: currentPage, pageCount, visibleItems: visiblePuzzles, setPage } = usePagination(filteredPuzzles, {
  anchor: '.collection-toolbar',
  filters: {
    category: { state: category, values: () => categories.value.map((item) => item.value) },
    ascending: { state: ascending },
  },
})
const solvedCount = computed(() => collectionPuzzles.value.filter((puzzle) => puzzle.status === 'solved').length)

function selectCategory(value: string) { category.value = value }
function toggleSort() { ascending.value = !ascending.value }
</script>
...
<template>
  <div v-if="collection" class="collection-detail shell-width">
    ...
    <PaginationControls id="collection-pagination" :page="currentPage" :page-count="pageCount" @change="setPage" />
